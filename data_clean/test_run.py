...
Respond with 'VERIFIED' if the data looks consistent and properly formatted, or 'CONCERN: [specific issue]' only if there's a clear data quality problem."""
    ...
    def update_csv_test(self, max_rows: int = 3):
        """Test version - processes only the first N rows"""
        # Read the CSV
        ...
        print(f"\n🃏 TEST RUN - Processing First {max_rows} Rows")
        print(f"📄 File: {self.csv_file}")
        print(f"📊 Total cards in file: {len(rows)}")
        ...
        for idx, row in enumerate(rows[:max_rows], 1):
            ...
            print(f"     Mana Cost: {extracted['mana_cost']}")
            print(f"     Power/Toughness: {extracted['power_toughness']}")
            print(f"     Card Text: {extracted['card_text'][:50]}..." if len(extracted['card_text']) > 50 else f"     Card Text: {extracted['card_text']}")
        ...
        print(f"📊 Would update {updated_count} cards")
        print(f"⚠️  NOTE: Changes NOT saved to file (test mode)")
        print(f"{'='*50}\n")
...
def main():
    ...
    # Run test with first 3 rows
    updater.update_csv_test(max_rows=3)
